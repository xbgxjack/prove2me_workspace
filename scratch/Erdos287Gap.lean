...
/-! ### 2-adic machinery (as used for Kürschák's block theorem) -/
...
/-! ### New: the 2-adic weight of an even block -/
...
/-! ### Structure of a gap-`≤ 2` representation -/
...
theorem last_not_prime (k : ℕ) (hk : 2 ≤ k) (f : ℕ → ℕ)
  ...
  exact no_large_prime k hk f hf1 hmono hsum _ hp (by omega) (k - 1) (by omega) rfl
...
private lemma no_odd_ap (k : ℕ) (hk : 2 ≤ k) (f : ℕ → ℕ)
  ...
  have hfi : f ((p - f 0) / 2) = p := by
    rw [hA _ hik]
    omega
  exact no_large_prime k hk f hf1 hmono hsum p hp hhigh _ hik hfi
