...
/-! This solution reduces to four already-Proved platform theorems
(`shannonEntropy_pi_le`, `shannonEntropy_pigeonhole`,
`choose_sum_le_exp_mul_binEntropy`, `kleitman_diameter`), imported above,
and otherwise reproduces Rothvoss's exposition of Spencer's entropy /
partial-coloring method (see the `source` field) end to end: a Pinsker-type
bound on the binary entropy function, the per-row "shell" entropy bound
(Lemma 9), the joint pigeonhole-and-Kleitman assembly for one round
(Lemma 8), the numeric telescoping-sum bound for the outer geometric
iteration, and the strong-induction construction combining them. -/
